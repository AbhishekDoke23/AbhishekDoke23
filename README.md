<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&pause=1000&color=FF9900&center=true&vCenter=true&width=700&lines=Hi+%F0%9F%91%8B%2C+I'm+Abhishek+Doke;Data+Engineer+%7C+AWS+Cloud+Specialist;Building+Scalable+Data+Pipelines+%E2%9A%A1;PySpark+%7C+Medallion+Architecture+%7C+Delta+Lake" alt="Typing SVG" />

<br/>

<img src="https://komarev.com/ghpvc/?username=AbhishekDoke23&label=Profile+Views&color=FF9900&style=flat-square" alt="Profile Views" />
&nbsp;
<a href="mailto:abhishekdoke23@outlook.com"><img src="https://img.shields.io/badge/Email-abhishekdoke23%40outlook.com-FF9900?style=flat-square&logo=microsoft-outlook&logoColor=white" /></a>
&nbsp;
<a href="https://www.linkedin.com/in/abhishek-doke-/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white" /></a>

</div>

---

## 👨‍💻 About Me

I'm a **Computer Engineering undergraduate** from Savitribai Phule Pune University (CGPA: **8.95/10**) with hands-on experience building **end-to-end data pipelines** and **cloud-native data infrastructure on AWS**.

- 🔭 I build **serverless ETL pipelines** using AWS Glue, Step Functions, and PySpark
- ☁️ Deep expertise in **AWS** — Lambda, S3, Athena, Glue, CloudWatch, SNS, IAM, EC2
- 🏗️ Strong believer in **Medallion Architecture** (Bronze → Silver → Gold) for clean data lakes
- ⚡ Experience with **Databricks** — Delta Lake, PySpark optimisation, SCD Type 1 & 2
- 🎓 **Oracle Cloud Certified** — AI Foundations & OCI Foundations (2025)
- 📍 Based in Pune, Maharashtra, India

---

## 🛠️ Tech Stack

### ☁️ AWS Cloud
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=FF9900)
![AWS Glue](https://img.shields.io/badge/AWS%20Glue-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![AWS Lambda](https://img.shields.io/badge/AWS%20Lambda-FF9900?style=for-the-badge&logo=awslambda&logoColor=white)
![AWS Step Functions](https://img.shields.io/badge/Step%20Functions-FF4F8B?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Amazon S3](https://img.shields.io/badge/Amazon%20S3-569A31?style=for-the-badge&logo=amazons3&logoColor=white)
![Amazon Athena](https://img.shields.io/badge/Amazon%20Athena-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=FF9900)
![CloudWatch](https://img.shields.io/badge/CloudWatch-FF4F8B?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Amazon SNS](https://img.shields.io/badge/Amazon%20SNS-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![IAM](https://img.shields.io/badge/AWS%20IAM-DD344C?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Oracle Cloud](https://img.shields.io/badge/Oracle%20Cloud-F80000?style=for-the-badge&logo=oracle&logoColor=white)

### 🔢 Data Engineering
![Apache Spark](https://img.shields.io/badge/Apache%20Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=for-the-badge&logo=databricks&logoColor=white)
![Delta Lake](https://img.shields.io/badge/Delta%20Lake-003366?style=for-the-badge&logo=databricks&logoColor=white)
![Apache Parquet](https://img.shields.io/badge/Apache%20Parquet-50ABF1?style=for-the-badge&logo=apache&logoColor=white)
![Amazon QuickSight](https://img.shields.io/badge/QuickSight-8C4FFF?style=for-the-badge&logo=amazon-aws&logoColor=white)

### 💻 Languages & Databases
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)

### 🔧 DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

## 🚀 Featured Projects

### 🎬 [End-to-End YouTube Trending Data Pipeline](https://github.com/AbhishekDoke23/youtube-trending-data-pipeline)
> **AWS · PySpark · Step Functions · Glue · Athena · QuickSight**

Built a production-grade **Medallion Data Lake** (Bronze/Silver/Gold) on Amazon S3 to process YouTube Data API streams and Kaggle datasets.

- ⚡ Reduced query latency by **60%** using Parquet-optimized Glue ETL jobs
- 💰 Cut storage costs by **80%** via S3 Lifecycle Policies → Glacier
- 🔁 Automated orchestration with **AWS Step Functions** + Data Quality Gate (Lambda)
- 📊 Real-time monitoring via **Amazon SNS** alerts

---

### 🤖 [Generative AI Backend — AWS SageMaker](https://github.com/AbhishekDoke23/genai-sagemaker-backend)
> **Hugging Face Falcon-7B · SageMaker · Lambda · API Gateway · CloudFront**

Serverless GenAI backend deploying **Hugging Face Falcon-7B-Instruct** for text summarization.

- 🔌 REST API via **Lambda + API Gateway** with low-latency inference
- 🔒 Secured with custom **IAM policies** + CloudFront + S3 bucket policies
- 🌐 CloudFront-hosted web frontend for end-to-end demo

---

### 🖥️ [Automated Cloud Monitoring & Self-Healing EC2](https://github.com/AbhishekDoke23/ec2-auto-recovery)
> **AWS EC2 · Bash · CloudWatch · SNS · IAM · S3**

Auto-recovery system that detects and heals EC2 failures with zero manual intervention.

- 🛠️ Bash scripts for **automated instance recovery** on failure detection
- 📡 **CloudWatch + SNS** for real-time alerting and incident notifications
- 🔐 Least-privilege **IAM roles** + S3 log storage

---

### 🏗️ [Databricks Medallion Architecture](https://github.com/AbhishekDoke23/databricks-medallion-architecture)
> **Databricks · PySpark · Delta Lake · SCD Type 1 & 2**

Enterprise-grade data lakehouse with **Bronze/Silver/Gold** layers, Star Schema, and Slowly Changing Dimensions.

---

### 🗄️ [SQL Data Warehouse on Databricks](https://github.com/AbhishekDoke23/databricks-sql-datawarehouse)
> **Databricks · SQL · Star Schema · Incremental Loading · SCD Type 1**

3-layer DWH with incremental loading patterns and optimized query performance.

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=AbhishekDoke23&show_icons=true&theme=dark&include_all_commits=true&count_private=true&title_color=FF9900&icon_color=FF9900&border_color=FF9900&hide_border=false" alt="Abhishek's GitHub Stats" />

</div>

<div align="center">

<img src="https://streak-stats.demolab.com?user=AbhishekDoke23&theme=dark&ring=FF9900&fire=FF9900&currStreakLabel=FF9900&border=FF9900" alt="GitHub Streak" />

</div>

---

## 🏅 Certifications

| Badge | Certification |
|-------|--------------|
| ☁️ | **Oracle Cloud Infrastructure 2025 Certified AI Foundations Associate** |
| 🏗️ | **Oracle Cloud Infrastructure 2025 Foundations Associate** |

---

## 📫 Let's Connect

<div align="center">

<a href="mailto:abhishekdoke23@outlook.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=microsoft-outlook&logoColor=white" />
</a>
&nbsp;
<a href="https://www.linkedin.com/in/abhishek-doke-/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
&nbsp;
<a href="https://github.com/AbhishekDoke23" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=FF9900&height=100&section=footer" />
</div>
